...
public class BrassChestplateItem extends ArmorItem {
    ...
    public BrassChestplateItem(Properties props) {
        super(ModArmorMaterials.BRASS, Type.CHESTPLATE,
    ...
    // FIXED: Fractional air consumption with debt accumulation
    public static void consumeAir(ItemStack stack, int amount) {
        ...
        float airDebt = tag.getFloat("AirDebt"); // Accumulator for fractional costs
        ...
        if (ArmorUpgradeHelper.hasUpgrades(s)) {
            ...
            if (powerCells > 0) {
                tooltip.add(Component.literal("⚡ Power Cells: " + powerCells + "/" + ArmorUpgradeHelper.MAX_POWER_CELLS
                        + " (+" + (powerCells * 250) + " power)").withStyle(ChatFormatting.YELLOW));
            }
